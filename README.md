...
<p align="center">
  <img src="https://img.shields.io/badge/version-1.0.42-d4779a?style=flat-square" alt="Version" />
...
- Composition-first, then document bundle submission; the Composition is now created exclusively through the Document Bundle POST to prevent HAPI-2840 duplicate resource errors, and all bundle entry resources use bundle-scoped UUIDs instead of server-assigned IDs for safer resubmission across all prescription templates
- The final Creator step can export the assembled FHIR Bundle as local JSON, written in a self-contained `urn:uuid:<v4>` form with matching internal references, IG-aligned cardinality (Patient `person-age`, Organization fallback contact, Encounter `serviceType`, Condition `category` and ICD-10-CM coding, Observation Vital Signs category and UCUM code, MedicationRequest TW-Core required fields), and en-US canonical display strings on standard codings — so the file passes the TW Core EMR `Bundle-EP` validator without manual editing
...
- Bundle detail now includes an **Export dropdown** with three formats: FHIR JSON, Postman Collection v2.1, and HTML report. The FHIR JSON export converts the HAPI-stored Document Bundle into a self-contained `urn:uuid` form with matching internal references, attaches `Reference.type` so external validators can resolve slice profile choices, strips HAPI-injected `meta.source` / `versionId` / `lastUpdated`, and removes extensions whose definitions are absent from the current TW Core EMR validator package — so any saved bundle can be re-validated offline against the IG without manual fixup
...
- **Result sort and filter**: a sort dropdown (Newest First / Oldest First / Name A→Z / Name Z→A / Bundle ID newest→oldest / Bundle ID oldest→newest) and a keyword filter input appear in the results header once results are loaded; Bundle ID sorting uses `Intl.Collator` with `numeric: true` so HAPI's numeric Bundle IDs order naturally while UUID-style IDs fall back to lexicographic order; the filter matches against patient name, identifier, and organization simultaneously; the filter resets automatically on each new search while the sort preference is retained across queries; a "no match" empty state appears when the filter eliminates all results
