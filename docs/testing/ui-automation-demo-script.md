...
這份展示稿的定位是「證明 RxFHIR 已經有真實的全自動 UI 驗證」，不是取代
App 內建的 **Live Demo** 或 **Feature Showcase**。  
如果你要做人看得懂、節奏較慢的產品導覽，優先使用 Live Demo / Feature
Showcase；如果你要證明自動化真的存在，再用這份腳本搭配 `test:ui:headed`。

如果你要現場展示自動化驗證，建議先準備好：
...
目前這一版已經是 **適合學生專題展示的交付版本**，重點是穩定、可展示、可重複，不是持續擴充更多低價值案例，也不是把正式測試改造成慢速影片播放器。
...
- 如果要慢節奏講產品故事，應該切回 Live Demo 或 Feature Showcase
...
- 結果列表有沒有出現
- detail pane 有沒有正常打開
- 稽核卡片有沒有立即顯示本機規則與 server fallback 結果
...
- 搜尋成功後的結果列表
- 點選結果後的 detail pane
- detail pane 裡的 Bundle audit 是否會顯示
- Structured / JSON 切換
- Diff dialog
- Export dropdown
...
## 案例 10：Creator preview handoff

### 你可以這樣講

這個案例驗證的是：即使不真的把 Bundle 送到 FHIR Server，Creator 最後一步也能把目前組裝好的 Bundle 直接交給 Consumer 預覽。

它會驗證：

- Creator 的最終 Composition 頁面可以直接跳到 Consumer
- Consumer 會以 preview 模式打開 detail pane
- 畫面會明確標示這份 Bundle 尚未提交
- 稽核卡片仍然會出現

### 為什麼重要

這對課堂 demo 很有價值，因為它降低了對 public HAPI 可寫入狀態的依賴，也更能證明 Creator 與 Consumer 之間的整合不是只靠成功提交後才成立。

---

## 案例 11：Consumer workspace persistence

### 你可以這樣講

這個案例驗證的是 Consumer 不是每次重開都從空白開始，而是會把上一次的查詢工作區接回來。

它會驗證：

- 上一次的搜尋條件會保留
- 重新進入 Consumer 會自動重新載入結果
- 上一次選取的 Bundle detail 會再次打開

### 為什麼重要

這證明 RxFHIR 已經不是單次展示型介面，而是有真正桌面工作區概念的應用程式。

---

## 案例 12：Settings server capability readiness

### 你可以這樣講

這個案例驗證 Settings 不只會顯示「有沒有連上」，還會顯示這台 server 是否看起來支援 `Bundle $validate`。

它會驗證：

- `/metadata` 測試連線是否成功
- Server 名稱與 FHIR 版本是否顯示
- capability badge 是否正確顯示 `Bundle $validate`

### 為什麼重要

這可以讓老師直接看到：RxFHIR 不只是連到 server 而已，還會評估這台 server 適不適合做 validation-heavy 的展示。

---

## 案例 13：OS-level open file handoff

### 你可以這樣講

這個案例驗證的是：如果作業系統或啟動參數直接給一個本機 Bundle JSON 檔案，RxFHIR 會直接進 Consumer 打開它。

它會驗證：

- Electron 啟動時能接收到本機檔案路徑
- App shell 會直接導到 Consumer
- detail pane 會自動打開並顯示內容

### 為什麼重要

這代表 RxFHIR 的桌面整合不只停在 App 內部按鈕，而是已經接到真正的桌面開檔生命週期。
...
- 執行 **17 個 spec files、26 個 UI 測試案例**
